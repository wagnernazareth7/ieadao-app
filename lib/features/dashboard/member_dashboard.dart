...
import '../highlights/widgets/live_now_banner.dart';
...
class MembroDashboard extends ConsumerWidget {
  ...
  ({String channel, String name, IconData icon, Color color}) _getHomogeneousGroup(dynamic user) {
    ...
    } catch (e) { debugPrint('Erro no cálculo de grupo homogêneo: $e'); }
    return defaultGroup;
  }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text(
          'IEADAO Mobile', 
          style: TextStyle(color: AppColors.secondary, fontWeight: FontWeight.w900, letterSpacing: 0.5)
        ),
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () => context.push('/ia-assistant'),
          ),
          ...
          children: [
            Container(
              ...
              child: userAsync.when(
                data: (user) {
                  // BLINDAGEM SÉNIOR: Se não houver nome, usa o fragmento do e-mail
                  String displayName = 'MEMBRO';
                  if (user != null) {
                    if (user.firstName.isNotEmpty) {
                      displayName = user.firstName.toUpperCase();
                    } else {
                      displayName = user.email.split('@')[0].toUpperCase();
                    }
                  }
                  
                  final treat = (user?.gender == 'Feminino' || user?.gender == 'female') ? 'irmã' : 'irmão';
                  
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('A Paz do Senhor $treat,', style: TextStyle(color: Colors.white.withValues(alpha: 0.7), fontSize: 14, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
                      Text('$displayName!', style: AppTextStyles.heading.copyWith(color: Colors.white, fontSize: 28)),
                ...
                children: [
                  ...
                  GridView.count(
                    ...
                    children: [
                      ...
                      _CompactMenu(icon: Icons.home_repair_service, label: 'Visitas', color: Colors.deepOrange, onTap: () => context.push('/solicitar-visita')),
  ...
  Widget _buildMuralCard(BuildContext context) {
    return Card(
      ...
      child: InkWell(
        ...
        child: Column(
          children: [
            Container(height: 70, width: double.infinity, decoration: const BoxDecoration(gradient: LinearGradient(colors: [Colors.blue, AppColors.primary])), child: const Icon(Icons.auto_awesome, color: Colors.white, size: 28)),
            const ListTile(dense: true, title: Text('Mural do Culto', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)), subtitle: Text('Fotos e vídeos recentes', style: TextStyle(fontSize: 11)), trailing: Icon(Icons.chevron_right, size: 16)),
          ],
        ),
      ),
    );
  ...
  Widget _buildGroupCard(BuildContext context, dynamic group) {
    return Card(
      ...
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16), side: BorderSide(color: group.color.withOpacity(0.2))),
      child: ListTile(dense: true, leading: Icon(group.icon, color: group.color, size: 22), title: Text(group.name, style: TextStyle(fontWeight: FontWeight.bold, color: group.color, fontSize: 13)), subtitle: const Text('Comunicação e serviço', style: TextStyle(fontSize: 11)), trailing: const Icon(Icons.chevron_right, size: 16), onTap: () => context.push('/chat/${group.channel}/${group.name}')),
    );
  }

  void _showSettings(BuildContext context, WidgetRef ref) {
    showModalBottomSheet(context: context, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (context) => Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [ListTile(leading: const Icon(Icons.person_outline), title: const Text('Meu Perfil'), onTap: () { Navigator.pop(context); context.push('/profile'); }), const Divider(), ListTile(leading: const Icon(Icons.logout, color: Colors.redAccent), title: const Text('Sair', style: TextStyle(color: Colors.redAccent)), onTap: () async { Navigator.pop(context); await AuthService().logout(); })])));
  }
}
...
class _CompactMenu extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => InkWell(onTap: onTap, borderRadius: BorderRadius.circular(12), child: Column(mainAxisSize: MainAxisSize.min, children: [Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: color.withOpacity(0.1), borderRadius: BorderRadius.circular(14)), child: Icon(icon, color: color, size: 20)), const SizedBox(height: 6), Text(label, style: const TextStyle(fontSize: 9, fontWeight: FontWeight.w600, color: AppColors.textSecondary), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis)]));
}
